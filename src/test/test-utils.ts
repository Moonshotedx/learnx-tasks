...
export class MockQueryEngine {
    ...
    async query(sql: string, params: any[] = []): Promise<{ rows: any[] }> {
        ...
        if (
            sqlLower.includes('join "course-runs" cr on cr.id') &&
            sqlLower.includes('join groups g')
        ) {
            ...
            if (isDebug) {
                console.log(
                    '🔍 Complex join query detected - courseActivityId:',
                    courseActivityId,
                    'runId:',
                    runId,
                );
            ...
            const courseActivity = this.data.course_activities.find(
                (ca: any) => ca.id === courseActivityId,
            );
            if (!courseActivity) {
                if (isDebug)
                    console.warn(
                        `⚠️  CourseActivity ${courseActivityId} not found`,
                    );
                return { rows: [] };
            }

            const activity = this.data.activities.find(
                (a: any) => a.id === courseActivity.activity_id,
            );
            const course = this.data.courses.find(
                (c: any) => c.id === courseActivity.course_id,
            );
            ...
            if (!activity || !course || !run) {
                if (isDebug)
                    console.warn(
                        `⚠️  Missing data - activity: ${!!activity}, course: ${!!course}, run: ${!!run}`,
                    );
                return { rows: [] };
            }

            const group = this.data.groups.find(
                (g: any) => g.id === run.group_id,
            );
            ...
            if (isDebug) {
                console.log(
                    '✅ Found activity with run info - group_id:',
        ...
        if (
            sqlLower.includes('from activities a') &&
            sqlLower.includes('join "course-activities" ca') &&
            sqlLower.includes('where ca.id')
        ) {
            const courseActivityId = params[0];
            const courseActivity = this.data.course_activities.find(
                (ca: any) => ca.id === courseActivityId,
        ...
        if (
            sqlLower.includes('from "course-runs"') &&
        ...
        if (
            sqlLower.includes('from "group-members" gm') &&
            sqlLower.includes("gm.role = 'student'")
        ) {
            const groupId = params[0];
            const members = this.data.group_members.filter(
                (gm: any) => gm.group_id === groupId && gm.role === 'student',
            );
            const students = members
                .map((m: any) => {
                    const user = this.data.users.find(
                        (u: any) => u.id === m.user_id,
                    );
        ...
        // Get course managers
        if (
            sqlLower.includes('from course_managers cm') &&
            sqlLower.includes('where cm.course_id')
        ) {
            const courseId = params[0];
            const managers = this.data.course_managers.filter(
                (cm: any) => cm.course_id === courseId,
            );
        ...
        if (
            sqlLower.includes('from "course-activities" ca') &&
            sqlLower.includes('where ca.activity_id')
        ) {
            const activityId = params[0];
            const courseActivities = this.data.course_activities.filter(
                (ca: any) => ca.activity_id === activityId,
            );
            const results = courseActivities.map((ca: any) => {
                const course = this.data.courses.find(
                    (c: any) => c.id === ca.course_id,
                );
                return {
        ...
        if (sqlLower.includes('from users') && sqlLower.includes('where id')) {
            ...
            return user
                ? { rows: [{ id: user.id, name: user.name }] }
                : { rows: [] };
        }

        // Get group info
        if (
            sqlLower.includes('from groups g') &&
            sqlLower.includes('where g.id')
        ) {
            const groupId = params[0];
            const group = this.data.groups.find((g: any) => g.id === groupId);
            if (!group) return { rows: [] };

            const run = this.data.course_runs.find(
                (r: any) => r.group_id === groupId,
            );
            const course = run
                ? this.data.courses.find((c: any) => c.id === run.course_id)
                : null;
        ...
        if (
            sqlLower.includes('from "course-runs" cr') &&
            sqlLower.includes('join courses c')
        ) {
            const runId = params[0];
            const run = this.data.course_runs.find((r: any) => r.id === runId);
            if (!run) return { rows: [] };
        ...
        if (
            sqlLower.includes('from assignment_submissions') &&
            sqlLower.includes('where activity_id')
        ) {
            const activityId = params[0];
            const runId = params[1];
            const submissions = this.data.assignment_submissions.filter(
                (s: any) =>
                    s.activity_id === activityId && s.course_run_id === runId,
            );
            return {
                rows: submissions.map((s: any) => ({ user_id: s.user_id })),
            };
        }

        // Get quiz attempts
        if (
            sqlLower.includes('from "quiz-attempts"') &&
            sqlLower.includes('where activity_id')
        ) {
            const activityId = params[0];
            const runId = params[1];
        ...
        if (
            sqlLower.includes('from exam_submissions') &&
            sqlLower.includes('where activity_id')
        ) {
            const activityId = params[0];
            const runId = params[1];
    ...
    printSummary(): void {
        ...
        for (const [testName, testResults] of byTest) {
            ...
            for (const result of testResults) {
                ...
                console.log(
                    `${resultIcon} ${result.scenario}: ${result.message}`,
                );
