...
async function testStudentDeadlineNotification() {
  ...
  testReporter.addResult(testName, 'Push Notification Recipients', pushResult);
  ...
  testReporter.addResult(testName, 'Email Notification Recipients', emailResult);
  ...
  for (const userId of scenario.expected_recipients) {
    ...
    testReporter.addResult(testName, `Content Validation for ${userId}`, contentResult);
  }
}
...
async function testRedoEnabledNotification() {
  ...
  const countResult = testAssertions.assertNotificationCount(userId, 1, 'push');
...
async function testMissedDeadlineNotification() {
  ...
  for (const userId of scenario.not_expected) {
    const countResult = testAssertions.assertNotificationCount(userId, 0, 'push');
...
async function testGroupIsolationStudentDeadline() {
  ...
  console.log(`  ℹ️  Found ${studentsRes.rows.length} students in Group ${groupId}`);
  ...
  testReporter.addResult(testName, 'Push Notification Group Isolation', pushResult);
  ...
  testReporter.addResult(testName, 'Email Notification Group Isolation', emailResult);
  ...
  console.log(`  ✅ Group isolation validated: ${scenario.expected_recipients.length} students in Group ${groupId} notified, ${scenario.not_expected.length} students from other groups excluded`);
}
...
async function testGroupIsolationActivityPosted() {
  ...
  console.log(`  ℹ️  Run ${runId} (${runName}) belongs to Group ${groupId} (${groupName})`);
...
async function testManagerCourseIsolationDeadlineWarning() {
  ...
  const scenario = mockData.test_scenarios.manager_course_isolation_deadline_warning;
  ...
  console.log(`  ℹ️  Found ${managersRes.rows.length} managers for Course ${courseId}`);
  ...
  testReporter.addResult(testName, 'Push Notification Manager Isolation', pushResult);
  ...
  testReporter.addResult(testName, 'Email Notification Manager Isolation', emailResult);
...
async function testManagerCourseIsolationMultipleCourses() {
  ...
  const scenario = mockData.test_scenarios.manager_course_isolation_multiple_courses;
  ...
  console.log(`  ℹ️  Found ${managersRes.rows.length} manager(s) for Course ${courseId}: ${managersRes.rows.map(m => m.id).join(', ')}`);
  ...
  testReporter.addResult(testName, 'Push Notification Manager Course Isolation', pushResult);
  ...
  testReporter.addResult(testName, 'Email Notification Manager Course Isolation', emailResult);
  ...
  console.log(`  ✅ Manager receives notifications only for their enrolled courses`);
}
...
async function testCrossGroupNoLeakageScores() {
  ...
  testReporter.addResult(testName, 'Push Notification No Cross-Group Leakage', pushResult);
  ...
  testReporter.addResult(testName, 'Email Notification No Cross-Group Leakage', emailResult);
...
async function testFacilitatorPostDeadlineSummaryIsolation() {
  ...
  const scenario = mockData.test_scenarios.facilitator_post_deadline_summary_isolation;
  ...
  console.log(`  ℹ️  Found ${facilitatorsRes.rows.length} facilitator(s) for Course ${courseId}: ${facilitatorsRes.rows.map(f => f.id).join(', ')}`);
  ...
  testReporter.addResult(testName, 'Push Notification Facilitator Isolation', pushResult);
  ...
  testReporter.addResult(testName, 'Email Notification Facilitator Isolation', emailResult);
  ...
  console.log(`  ✅ Facilitator course isolation validated for post-deadline summary`);
}
...
async function testMultipleGroupsOneStudent() {
  ...
  console.log(`  ℹ️  Found ${studentsRes.rows.length} student(s) in Group ${groupId}: ${studentsRes.rows.map(s => s.id).join(', ')}`);
  console.log(`  ℹ️  Note: user-student-3 is in BOTH Group 1 and Group 3, but should only receive notification for Group 3 context (Run 4)`);
  ...
  testReporter.addResult(testName, 'Push Notification Multi-Group Student Isolation', pushResult);
  ...
  testReporter.addResult(testName, 'Email Notification Multi-Group Student Isolation', emailResult);
  ...
  const student3CountResult = testAssertions.assertNotificationCount('user-student-3', 1, 'push');
  testReporter.addResult(
    testName,
  ...
  console.log(`  ✅ Student in multiple groups receives notifications correctly based on run context`);
}
